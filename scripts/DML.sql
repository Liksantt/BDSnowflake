...
SELECT store_country, store_state, store_city, NULL FROM mock_data WHERE store_city IS NOT NULL
UNION SELECT supplier_country, NULL, supplier_city, NULL FROM mock_data WHERE supplier_city IS NOT NULL
UNION SELECT customer_country, NULL, NULL, customer_postal_code FROM mock_data WHERE customer_postal_code IS NOT NULL
UNION SELECT seller_country, NULL, NULL, seller_postal_code FROM mock_data WHERE seller_postal_code IS NOT NULL;
...
SELECT DISTINCT product_brand FROM mock_data WHERE product_brand IS NOT NULL AND product_brand != ''
ON CONFLICT (brand_name) DO NOTHING;

INSERT INTO dim_pets (pet_type, pet_name, pet_breed, pet_category) 
SELECT DISTINCT customer_pet_type, customer_pet_name, customer_pet_breed, pet_category FROM mock_data WHERE customer_pet_type IS NOT NULL
...
FROM mock_data WHERE customer_email IS NOT NULL AND customer_email != ''
...
FROM mock_data WHERE seller_email IS NOT NULL AND seller_email != ''
ON CONFLICT (email) DO NOTHING;
...
FROM mock_data WHERE product_name IS NOT NULL AND product_name != ''
...
FROM mock_data WHERE store_name IS NOT NULL AND store_name != ''
...
FROM mock_data WHERE supplier_name IS NOT NULL AND supplier_name != ''
